type FacilityCategory = "medical" | "dental" | "pharmacy";
...
interface CompactFacility {
  name: string;
  address: string | null;
  category: FacilityCategory;
  sourceIds: string[];
  standards: StandardTuple[];
}

interface FacilityShard {
  schemaVersion: 1;
  asOf: string;
  facilities: Record<string, CompactFacility>;
}
...
let catalogPromise: Promise<StandardCatalog> | null = null;
const shardPromises = new Map<string, Promise<FacilityShard>>();
...
function isValidMedicalInstitutionCode(value: string): boolean {
...
function renderSources(sources: SourceEntry[]): void {
  const body = requiredElement<HTMLTableSectionElement>("source-list");
...
async function loadMetadata(): Promise<void> {
  const state = requiredElement<HTMLElement>("metadata-state");
  try {
    manifest = await fetchJson<DataManifest>(`${DATA_ROOT}/manifest.json`);
    requiredElement("overall-as-of").textContent = formatDate(manifest.asOf);
    requiredElement("generated-at").textContent = formatDateTime(
...
function loadSearchIndex(): Promise<SearchIndex> {
  if (!searchIndexPromise) {
    searchIndexPromise = fetchJson<SearchIndex>(`${DATA_ROOT}/search.json`);
...
function loadCatalog(): Promise<StandardCatalog> {
  if (!catalogPromise) {
    catalogPromise = fetchJson<StandardCatalog>(`${DATA_ROOT}/catalog.json`);
...
function loadShard(code: string): Promise<FacilityShard> {
  const prefix = code.slice(0, 4);
  const existing = shardPromises.get(prefix);
  if (existing) {
    return existing;
  }
  const request = fetchJson<FacilityShard>(
    `${DATA_ROOT}/facilities/${prefix}.json`,
  );
...
function setSearchState(message: string): void {
  requiredElement("search-state").textContent = message;
}

function clearResults(): HTMLElement {
...
function sourceLinks(sourceIds: string[]): HTMLElement {
...
async function showFacility(code: string): Promise<void> {
  ...
  try {
    ...
    meta.append(
      document.createTextNode(`医療機関コード ${code}`),
      document.createTextNode(facility.address ?? "住所情報なし"),
      document.createTextNode(`基準日 ${formatDate(shard.asOf)}`),
    );
      ...
      for (const [standardId, acceptanceNumber, effectiveFrom] of facility.standards) {
        ...
        copy.append(
          element(
            "div",
            "standard-name",
            definition?.name ?? definition?.abbreviation ?? "名称未収録",
          ),
        );
        if (definition?.abbreviation) {
          copy.append(
            element("div", "standard-abbr", `略称：${definition.abbreviation}`),
          );
        }
        const dates = document.createElement("div");
        dates.append(
          element("div", undefined, `受理番号：${acceptanceNumber}`),
          element(
            "div",
            "standard-date",
            effectiveFrom
...
function resultCard(tuple: SearchTuple): HTMLElement {
  ...
  meta.append(
    document.createTextNode(`医療機関コード ${code}`),
    document.createTextNode(address ?? "住所情報なし"),
  );
...
async function searchByName(query: string): Promise<void> {
  const results = clearResults();
  ...
  try {
    ...
    if (matches.length === 0) {
      results.append(
        element("div", "empty-state", "一致する医療機関が見つかりませんでした。"),
      );
    ...
    const suffix =
      matches.length > RESULT_LIMIT
        ...
        : "";
    setSearchState(
      `「${query}」に一致する施設は${formatNumber(matches.length)}件です${suffix}。`,
    );
  } catch (error) {
    console.error(error);
    showError(
